...
{  
	...
	public function update_bunker_supply($bunker_id,$supply_date,$suppliers,$port_of_supply,$mdo,$hfo,$luboil_1_type,$luboil_1_quantity,$luboil_2_type,$luboil_2_quantity,$others,$remarks,$reminder,$date_due,$invoice_amount,$invoice_num,$currency,$document1,$document2,$paid,$paid_date)
	{
              $this->db->query("UPDATE bunker_supply SET date_of_supply='$supply_date' ,suppliers='$suppliers' ,due_date='$date_due',port_of_supply='$port_of_supply',mdo='$mdo',hfo='$hfo',luboil_1_type='$luboil_1_type',reminder='$reminder',luboil_1_quantity='$luboil_1_quantity',luboil_2_type='$luboil_2_type',luboil_2_quantity='$luboil_2_quantity',others='$others',remarks='$remarks',invoice_amount='$invoice_amount',invoice_num='$invoice_num',currency='$currency',document1='$document1',document2='$document2',paid='$paid',paid_date='$paid_date' WHERE bunker_id='$bunker_id' ");
             return true;
	}
        ...
        if($match_values){
		    ...
		    if($custom_code == "all")
		      ...
		      if($custom_code == "red")
			  {
				if(empty($where))
				{
					$where .=	" WHERE ( due_date <= CURDATE()+ INTERVAL 5 day AND paid='No' ) AND vessel_id= '$vessel_id'";	
				}
				else
				{
					$where .=	"  IF( UNIX_TIMESTAMP(`due_date`) <>0 ,due_date < CURDATE()+ INTERVAL 5 day ) AND vessel_id= '$vessel_id'";
