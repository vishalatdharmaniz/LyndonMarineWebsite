	...
	public function getSheetLog($getfields='', $match_values = '',$condition ='', $compare_type = '', $count = '',$num = '',$offset='',$orderby='',$orderbyfield='',$and_match_value='', $custom_code = "", $start_date = "", $last_date = "", $vessel_id)
    {
		...
		/*
			if($custom_code == "brown"){
				if(empty($where)){
					$where .=	"WHERE ((extention_until <= CURRENT_DATE() + INTERVAL 30 day AND extention_until > CURDATE() AND extention_until!='' ) OR (date_expiry <= CURRENT_DATE() + INTERVAL 30 day AND date_expiry > CURDATE() AND extention_until='')) AND vessel_id='$vessel_id'";	
				}else{
			...
			if($custom_code == "yellow"){
				if(empty($where)){
					$where .=	" WHERE  ( (date_expiry > CURRENT_DATE() + INTERVAL 30 day AND date_expiry <= CURRENT_DATE() + INTERVAL 45 day) OR (extention_until > CURRENT_DATE() + INTERVAL 30 day AND extention_until <= CURRENT_DATE() + INTERVAL 45 day)) AND vessel_id= '$vessel_id'";	
				}else{
			...
			*/
			...
			if($custom_code == "green"){
				if(empty($where)){
					$where .=	"  WHERE ( (date_expiry >= CURRENT_DATE() + INTERVAL 46 day AND date_expiry !='') OR (extention_until >= CURRENT_DATE() + INTERVAL 46 day AND extention_until !='') ) AND vessel_id='$vessel_id'";	
				}else{
					$where .=	" IF( UNIX_TIMESTAMP(`date_expiry`) <>0 ,date_expiry >= CURRENT_DATE() + INTERVAL 46 day or extention_until > CURRENT_DATE() + INTERVAL 46 day ) AND vessel_id= '$vessel_id'";
				}
			}
			...
			if($custom_code == "yellow"){
				if(empty($where)){
					$where .=	" WHERE  ( (date_expiry >= CURRENT_DATE() + INTERVAL 31 day AND date_expiry < CURRENT_DATE() + INTERVAL 46 day AND extention_until = '') OR (extention_until >= CURRENT_DATE() + INTERVAL 31 day AND extention_until < CURRENT_DATE() + INTERVAL 46 day AND extention_until !='' )) AND vessel_id= '$vessel_id'";	
				}else{
					$where .=	"  WHERE  ( (date_expiry >= CURRENT_DATE() + INTERVAL 31 day AND date_expiry < CURRENT_DATE() + INTERVAL 46 day  AND extention_until = '') OR (extention_until >= CURRENT_DATE() + INTERVAL 31 day AND extention_until < CURRENT_DATE() + INTERVAL 46 day)) AND vessel_id= '$vessel_id'";
				}
			}
