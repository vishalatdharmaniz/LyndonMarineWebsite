...
class Company extends CI_Controller
 {
	...
	public function edit_company_user($company_id){
		...
		if($this->input->method() == "post"){
			echo "hi";die;
		$result_check = $this->Company_model->get_company($company_id);
			if(!empty($result_check) && $result_check[0]['email'] == $this->input->post("email")){
		...
		if($this->input->method() == "post"){
			$fname = $this->input->post("fname");
					...
					$this->Company_model->update_company_user($data,$company_id);
					redirect("admin/company/company_user/$company_id");
		}else{
			$result = $this->Company_model->get_company($company_id);
			if(!empty($result)){
