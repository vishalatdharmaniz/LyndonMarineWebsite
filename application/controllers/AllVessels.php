...
$config['full_tag_open'] = '<ul class="pagination">';
 ...
 //print_r($data["results"]);die();
	...
	public function searchdata($searchname)
	{
		...
		$all_vessels = $this->Vessel_model->search_vessel($searchname,$company_id);
		

		$data['role'] = $role_value[0]['role']; 
		$data['company_id'] = $company_id; //print_r($data['company_id']);
		$data['all_vessels'] = $all_vessels;
	...
	public function company_vessel($company_id)
	{
		...
		$role_value= $this->Company_model->get_role_by_company_id($company_id); 
		$all_vessels = $this->Vessel_model->get_vessel_data_by_company_id($company_id);
	    

		$role = $role_value[0]['role'];
		$data['role'] = $role; 
		
		$config = array();

       $config["base_url"] = base_url() . "index.php/AllVessels/company_vessel/$company_id";
       $config["total_rows"] = $this->Pagination_model->record_count($company_id); //print_r($config["total_rows"]);die();
