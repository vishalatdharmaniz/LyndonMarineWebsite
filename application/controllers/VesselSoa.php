 ...
 public function searchdata($searchname,$vessel_id,$offset=0)
    {
        ...
        $this->load->model('Vessel_model');
           ...
           $config['full_tag_open'] = '<ul class="pagination">';
            ...
            $soa_data = $this->Soa_model->searchtable($searchname,$vessel_id,$offset);
            $data['soa_data'] = $soa_data;
        
             $vessel_data = $this->Vessel_model->get_vessel_details_by_id($vessel_id);
             $data['vessel_data']= $vessel_data;

            $total_search_soa = $this->Soa_model->searchtable_total($searchname,$vessel_id);

            $config['base_url'] = base_url().'index.php/VesselSoa/searchdata/'.$searchname.'/'.$vessel_id;
            $config['total_rows'] = $total_search_soa;
            $config['per_page'] = 10;
